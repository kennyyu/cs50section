...
int main(int argc, char *argv[]) {
  ...
  /* this struct will contain the data we read from the file */
  pokemon current;

  /* the number of objects read; return value of fread */
  int num_read; 

  /* keep iterating through the file until we reach end-of-file */
  while ( !feof(infile) ) {
    /* Read a pokemon size's worth of bytes into current */
    num_read = fread(&current, sizeof(pokemon), 1, infile);

    /* This prevents the last line from being printed out twice
     * See: http://www.drpaulcarter.com/cs/common-c-errors.php#4.2 */
    if (num_read) {
